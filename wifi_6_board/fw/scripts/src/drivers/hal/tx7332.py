...
class TP_HAL_TX7332(TP_HAL):
    ...
    def get_cmd_sequence(self, from_history=False):
        ...
        for i in range(len(addr_list)):
            cmd_list.append(WriteTxReg(addr=addr_list[i], val=val_list[i]))
    ...
    def default_config(
        ...
        cmd_test = SwitchSpiMux(mux_code=3)

        # Make a command sequence
        cmd_seq = CommandSequence([cmd_test])
        ...
        cmd_seq.add_command(self.get_cmd_sequence())
