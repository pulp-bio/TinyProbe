...
class TP_HAL_FPGA(TP_HAL):
    ...
    def get_cmd_sequence(self, write_all_regs=False, from_history=False):
        ...
        for i in range(len(addr_list)):
            cmd_list.append(WriteFPGAReg(addr=addr_list[i], val=val_list[i]))

        # Extend the command sequence with the list of new commands
        result.add_command(cmd_list)
    ...
    def default_config(
        ...
        cmd_spi = SwitchSpiMux(mux_code=1)

        # Make a command sequence
        cmd_seq = CommandSequence([cmd_spi])
        ...
        cmd_seq.add_command(self.get_cmd_sequence(write_all_regs=True))
    ...
    def set_tgc_settings(
        ...
        # Make a command sequence
        cmd_seq.add_command(self.get_cmd_sequence(from_history=True))
        return cmd_seq
